...
workflow BINNING_PREPARATION {
    ...
    if (params.binning_map_mode == 'all') {
        ...
        ch_bowtie2_input = BOWTIE2_ASSEMBLY_BUILD.out.assembly_index.combine(reads)
    }
    else if (params.binning_map_mode == 'group') {
        // combine assemblies with reads of samples from same group
        ch_reads_bowtie2 = reads.map { meta, samplereads -> [meta.group, meta, samplereads] }
        ch_bowtie2_input = BOWTIE2_ASSEMBLY_BUILD.out.assembly_index
            ...
            .map { group, assembly_meta, assembly, index, reads_meta, samplereads -> [assembly_meta, assembly, index, reads_meta, samplereads] }
    }
    else {
        // i.e. --binning_map_mode 'own'
        // combine assemblies (not co-assembled) with reads from own sample
        ch_reads_bowtie2 = reads.map { meta, samplereads -> [meta.id, meta, samplereads] }
        ch_bowtie2_input = BOWTIE2_ASSEMBLY_BUILD.out.assembly_index
            .map { meta, assembly, index -> [meta.id, meta, assembly, index] }
            .combine(ch_reads_bowtie2, by: 0)
            .map { id, assembly_meta, assembly, index, reads_meta, samplereads -> [assembly_meta, assembly, index, reads_meta, samplereads] }
    }

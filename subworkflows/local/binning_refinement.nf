...
workflow BINNING_REFINEMENT {
    ...
    ch_bins = bins
        .map { meta, assembly_bins ->
            def meta_new = meta - meta.subMap(['domain', 'refinement'])
            [meta_new, assembly_bins]
        }
        .groupTuple()
        .map { meta, assembly_bins ->
            [meta, assembly_bins.flatten()]
        }
    ...
    ch_input_for_renamedastool = DASTOOL_DASTOOL.out.bins.map { meta, assembly_bins ->
        def domain_class = params.bin_domain_classification ? 'prokarya' : 'unclassified'
    ...
    refined_unbins = RENAME_POSTDASTOOL.out.refined_unbins.map { meta, assembly_bins ->
